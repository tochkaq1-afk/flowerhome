...
function initPreloader() {
  ...
  /* Заставка кончается примерно через 1,2с ПОСЛЕ ОТКРЫТИЯ СТРАНИЦЫ, а не
     после того, как доехал скрипт. На медленной связи скрипт приходит
     поздно, и раньше поверх этого ожидания накладывалась ещё вся заставка
     целиком — человек ждал дважды. Теперь на то, что уже прошло, она
     укорачивается сама; меньше 400мс не опускаем, иначе счётчик
     мелькнёт и пропадёт. */
  const DURATION = Math.max(400, 1200 - Math.round(performance.now()));
  const t0 = Date.now();
